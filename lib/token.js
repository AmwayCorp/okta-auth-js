...
function convertOAuthParamsToQueryParams(oauthParams) {
  ...
  // Loop through custom querystring parameters, populating only those that don't conflict with the standard OAuth parameters.
  var customParams = oauthParams.custom_querystring_parameters;
  if (customParams) {
    for (var prop in customParams) {
      if (customParams.hasOwnProperty(prop)) {
        var value = customParams[prop];
        if (value !== null && value !== undefined) {
          if (!oauthQueryParams[prop]) {
            oauthQueryParams[prop] = value;
          } else {
            throw new AuthSdkError('OAuth parameters may not be supplied through custom_querystring_parameters.');
          }
        }
      }
    }
